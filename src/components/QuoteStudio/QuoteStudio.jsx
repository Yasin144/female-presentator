...
function QuoteStudio({active=true}){
  ...
  const previewMusicGenerationRef=useRef(0);
  ...
  useEffect(()=>()=>{stopPreviewMusic();stopPreviewSpeech();},[]);
  ...
  const stopPreviewMusic=()=>{previewMusicGenerationRef.current+=1;try{previewSourceRef.current?.stop()}catch{}previewSourceRef.current=null;previewGainRef.current=null};
  const stopPreviewSpeech=()=>{if(!previewSpeechRef.current)return;previewSpeechRef.current=null;window.speechSynthesis?.cancel();};
  const startPreviewMusic=async(offset=time)=>{if(!active||!musicOn)return;stopPreviewMusic();const generation=previewMusicGenerationRef.current;const{ctx,buffer}=await ensureSoundtrack();if(generation!==previewMusicGenerationRef.current)return;const source=ctx.createBufferSource(),gain=ctx.createGain();source.buffer=buffer;gain.gain.value=clamp(volume/100,0,.45)*(autoDucking&&voiceOn?.72:1);source.connect(gain).connect(ctx.destination);source.start(0,clamp(offset+musicStart,0,29.95));previewSourceRef.current=source;previewGainRef.current=gain};

  useEffect(()=>{if(playing&&active&&musicOn)startPreviewMusic(time).catch(()=>{});else stopPreviewMusic();return()=>stopPreviewMusic()},[playing,active,musicOn,soundtrack.id]);
  useEffect(()=>{if(!active)setPlaying(false);if(!playing||!active||!voiceOn)stopPreviewSpeech();},[playing,active,voiceOn]);
  ...
  const speak=()=>{
    if(!active||!voiceOn||!('speechSynthesis'in window)) return;
    stopPreviewSpeech(); const u=new SpeechSynthesisUtterance(storyLines.join(' ')); u.rate=.9; u.pitch=voice.includes('Female')?1.08:.88;
    previewSpeechRef.current=u;
    u.onend=u.onerror=()=>{if(previewSpeechRef.current===u)previewSpeechRef.current=null;};
  ...
  const exportVideo=async(presetOverride=null,fpsOverride=null)=>{
    if(exporting)return;
    if(voiceOn){setExportStatus({phase:'Browser voice is preview-only and is not included in exported video. Turn off Preview voice to export the soundtrack-only video.',pct:0,error:true});return;}
    setExporting(true);setPlaying(false);setExportStatus({phase:'Building original 48 kHz soundtrack…',pct:2});
    try{
  ...
  return <div className="qs-shell">
    ...
    {voiceOn&&<div className="qs-export-status error" role="note"><b>Preview voice only</b><p>The browser voice is not recorded into exports. Turn it off to export your video with its soundtrack.</p><button onClick={()=>setVoiceOn(false)}>Turn off Preview voice for export</button></div>}
    <header className="qs-hero"><div><span className="qs-kicker">CREATE WITH FEELING</span><h1>Quote Studio</h1><p>Choose a thought, shape your story, then save a short video.</p></div><div className="qs-mode" aria-label="Quote editing mode">{['quick','pro','smart'].map(m=><button key={m} className={mode===m?'active':''} aria-pressed={mode===m} onClick={()=>setMode(m)}>{m==='smart'?'Smart creator':m==='quick'?'Simple':'More controls'}</button>)}</div></header>
    <main className="qs-grid">
      <section className="qs-panel qs-create">
        ...
        <div className="qs-personality-row">
          ...
          <label><span>Language (English only)</span><select value={language} disabled title="Local quote generation currently supports English only; translation is unavailable." onChange={e=>setLanguage(e.target.value)}>{LANGUAGES.map(x=><option key={x}>{x}</option>)}</select></label>
        </div>
        ...
        <button className="qs-generate" onClick={()=>generate(false)} disabled={busy}>{busy?generationStage:'Create a 30-second story'}</button>
        <div className="qs-master-actions"><button onClick={()=>generate(false,true)}>Create a polished version</button><button onClick={legendOfDay}>Thought of the day</button></div>
        ...
        {mode!=='quick'&&<div className="qs-pro-controls">
          <label>Preview text style (not exported)<select value={animation} onChange={e=>setAnimation(e.target.value)}><option value="soft-rise">Soft Rise</option><option value="word-reveal" disabled>Word Reveal — unavailable</option><option value="dramatic">Dramatic Punch</option><option value="typewriter">Soft Fade</option></select></label>
          <label>Preview voice style<select value={voice} onChange={e=>setVoice(e.target.value)}><option>Warm Female</option><option>Inspirational Female</option><option>Deep Male</option><option>Calm Male</option></select></label>
          <label className="qs-switch"><input type="checkbox" checked={voiceOn} onChange={e=>setVoiceOn(e.target.checked)}/> Preview voice (not exported)</label>
          ...
          <label>Creator series (unavailable)<select value={series} disabled title="This module generates one short at a time; automatic series generation is unavailable." onChange={e=>setSeries(e.target.value)}>{SERIES.map(x=><option key={x}>{x}</option>)}</select></label>
        </div>}
      ...
      <section className="qs-preview-column">
        ...
        <div className="qs-player"><button aria-label={playing?'Pause preview':'Play preview'} onClick={()=>{setPlaying(p=>!p);if(!playing)speak()}}>{playing?'❚❚':'▶'}</button><input aria-label="Preview position" type="range" min="0" max="30" step=".1" value={time} onChange={e=>{setPlaying(false);setTime(+e.target.value)}}/><span>{Math.floor(time)}s / 30s</span></div>
        <button className="qs-new-quote" onClick={()=>generate(false)} disabled={busy}>{busy?<><i/> {generationStage}</>:'Create another quote'}</button>
      ...
      <section className="qs-panel qs-editor">
        <div className="qs-step"><b>02</b><div><h2>Refine your story</h2><p>Select a scene to preview it. Edit the words below.</p></div></div>
        ...
        <label className="qs-label">Story text · one line per scene</label><textarea className="qs-edit" aria-label="Story text" value={storyLines.join('\n')} onChange={e=>setStoryLines(e.target.value.split('\n').map(x=>x.trim()).filter(Boolean).slice(0,16))}/>
      </section>
    </main>
    <section className="qs-timeline"><header><div><b>03</b><h2>30-second cinematic timeline</h2></div><span>All active media and audio layers</span></header><div className="qs-track media"><label>VIDEO</label><div>🎬 {theme.name} · native target-resolution composition</div></div><div className="qs-track"><label>TEXT</label>{scenes.map(s=><button key={s.start} onClick={()=>setTime(s.start)} style={{width:`${(s.end-s.start)/30*100}%`}}>{s.label}</button>)}</div><div className="qs-track media"><label>ANIM</label><div>✨ Preview style: {animation} · export uses static text</div></div>{voiceOn&&<div className="qs-track voice"><label>VOICE</label><div>🎙 {voice} · preview only, not exported · preview ducking {autoDucking?'on':'off'}</div></div>}{musicOn&&<div className="qs-track music"><label>MUSIC</label><div className="qs-miniwave">{waveform.map((v,i)=><i key={i} style={{height:`${Math.max(3,v/5)}px`}}/>)}<span>♫ {soundtrack.name} · {volume}%</span></div></div>}{soundFx&&<div className="qs-track sfx"><label>SFX</label><div>◉ Subtle whoosh · atmospheric rise · final resolve</div></div>}</section>
    <section className="qs-bottom">
      <div className="qs-publish">
        ...
        {publishingTab==='source'&&<div className="qs-kit-section qs-source-grid"><div><label>📚 QUOTE SOURCE</label><strong>{attribution?.name||'Original Wisdom'}</strong><p>{attribution?`“${attribution.quote}”`:'This script is original and is not attributed to a historical personality.'}</p><small>{attribution?.source||'Pattan Original Wisdom Engine'} · {attribution?.status||'✨ Original AI Quote'}</small></div><div><label>🎵 MUSIC LICENSE</label><strong>{soundtrack.name}</strong><p>{soundtrack.licenseType}</p><small>{soundtrack.commercialUse} · {soundtrack.attribution}</small></div><div><label>🎙 VOICE STATUS</label><strong>{voiceOn?voice:'Preview voice off'}</strong><p>{voiceOn?'Browser preview only; narration is not included in exports.':'Music-only export selected.'}</p></div><div><label>📊 QUALITY</label><strong>{score.overall}/100 Soul Score</strong><p>{freshness.score}% freshness · never used in your library</p></div><div><label>❤️ CTA TEXT</label><strong>{ctaIntensity.toUpperCase()}</strong><p>{ctaIntensity==='off'?'No CTA in video':ctaText}</p></div><div><label>🖼 COVER TEXT</label><strong>{packageData.coverText}</strong></div></div>}
        {publishingTab==='branding'&&<div className="qs-kit-section qs-brand-settings"><label>Channel Name<input value={brand.channelName} onChange={e=>setBrand(b=>({...b,channelName:e.target.value}))}/></label><label>Channel Handle<input value={brand.handle} onChange={e=>setBrand(b=>({...b,handle:e.target.value}))}/></label><label>Preferred CTA<input value={brand.preferredCta} onChange={e=>setBrand(b=>({...b,preferredCta:e.target.value}))} placeholder="Leave blank for Smart CTA"/></label><label>Brand font (unavailable)<select value={brand.brandFont} disabled title="Saved preference retained; exports currently use Arial, and preview fonts follow the selected style." onChange={e=>setBrand(b=>({...b,brandFont:e.target.value}))}><option>Inter</option><option>Poppins</option><option>Montserrat</option><option>Playfair Display</option></select></label><label>Outro style (unavailable)<select value={brand.outro} disabled title="Saved preference retained; selectable outros are not implemented." onChange={e=>setBrand(b=>({...b,outro:e.target.value}))}><option>Soft Fade</option><option>Cinematic Hold</option><option>Minimal Black</option></select></label><label className="qs-switch"><input type="checkbox" checked={brand.branding} onChange={e=>setBrand(b=>({...b,branding:e.target.checked}))}/> Show branding</label><label className="qs-switch"><input type="checkbox" checked={brand.watermark} disabled title="Saved preference retained; custom watermarks are not applied to the output." onChange={e=>setBrand(b=>({...b,watermark:e.target.checked}))}/> Watermark (not applied)</label><label className="qs-switch"><input type="checkbox" checked={brand.intro} disabled title="Saved preference retained; an intro is not added to the output." onChange={e=>setBrand(b=>({...b,intro:e.target.checked}))}/> Intro (unavailable)</label></div>}
      </div>
      <div className="qs-export-card"><span>SHORT QUALITY SCORE · {score.overall}/100</span><h2>04 · Export your video</h2><p>Review the quality and frame rate, then save your video as MP4.</p><div className="qs-export-options"><label>Quality<select value={exportPreset} onChange={e=>setExportPreset(e.target.value)}><option value="premium">1080 × 1920 — Premium</option><option value="ultra">2160 × 3840 — YouTube Ultra</option><option value="master">2160 × 3840 — Master File</option></select></label><label>Frame rate<select value={exportFps} onChange={e=>setExportFps(+e.target.value)}><option value="30">30 FPS — Cinematic</option><option value="60">60 FPS — Ultra Smooth (1080p)</option></select></label></div><div className="qs-license good">✓ Original soundtrack loaded · 48 kHz · 320 kbps final AAC · H.264 High Quality</div><button onClick={()=>exportVideo()} disabled={exporting}>{exporting?`${exportStatus?.phase||'Rendering…'} ${exportStatus?.pct||0}%`:'Export MP4'}</button><button className="qs-max-export" onClick={()=>exportVideo('master',30)} disabled={exporting}>Export 4K master · 30 FPS</button>{exportStatus&&<div className={`qs-export-status ${exportStatus.error?'error':''}`}><b>{exportStatus.phase}</b>{exportStatus.validation&&<span>{exportStatus.validation.width}×{exportStatus.validation.height} · {exportStatus.validation.videoCodec} · {exportStatus.validation.audioCodec} {exportStatus.validation.sampleRate} Hz · audio verified</span>}</div>}<small>MP4 · H.264 · AAC 48 kHz/320 kbps · no watermark</small></div>
    </section>
    <footer className="qs-history"><h2>Your quote library</h2><p>{history.length} generated · {published.length} excluded after export · {favorites.length} favorites · repeat checks help keep your stories varied</p></footer>
  </div>
}
